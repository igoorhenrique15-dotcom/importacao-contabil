(function(){
  ...
  function load(){try{const parsed=JSON.parse(localStorage.getItem(KEY));return parsed?.version===2?parsed:empty()}catch{return empty()}}
  let state=load();
  ...
  function createLot(data={}){const id=uid('lote'),now=new Date().toISOString();state.lots[id]={id,client:data.client||'',period:data.period||'',bank:data.bank||'',account:data.account||'',system:data.system||'',createdAt:now,updatedAt:now,currentStep:1,steps:{1:'in_progress',2:'pending',3:'pending',4:'pending',5:'pending',6:'pending',7:'pending',8:'pending'},records:[],dailyClosing:[],audit:[{id:uid('aud'),at:now,action:'Lote criado',detail:'Contexto inicial registrado'}],templates:{}};state.activeLotId=id;persist();return state.lots[id]}
  ...
  window.ContabilStore={uid,active,ensure,createLot,updateLot,addAudit,setRecords,setClosing,setStep,switchLot,removeLot,listLots,saveTemplate,getTemplate,get state(){return state}};
})();
