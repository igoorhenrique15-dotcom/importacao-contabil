(function(){
  ...
  function run(){
    ...
    try{let result,config={},state='complete';
      ...
      if(step===8){config={system:document.getElementById('layout-system').value};layoutResult=eng.buildLayout(base,config);result=layoutResult;if(!result.count)state='warning'}
  ...
  function table(headers,rows){return'<div class="table-wrap"><table class="result-table"><thead><tr>'+headers.map(h=>'<th>'+h+'</th>').join('')+'</tr></thead><tbody>'+rows.map(cols=>'<tr>'+cols.map(c=>'<td>'+c+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>'}
