(function(){
  ...
  function run(){
    ...
    try{let result,config={},state='complete';
      ...
      if(step===6){config={template:document.getElementById('history-template').value,client:lot.client,maxLength:Number(document.getElementById('history-max').value)||255};result=eng.generateHistory(base,config)}
      if(step===7){result=eng.validate(base,{period:lot.period});if(result.some(x=>x.validationStatus!=='valido'))state='warning'}
